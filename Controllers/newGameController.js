    ...
    if(matchData){
      if(matchData.members.indexOf(userId) !== -1){
        ...
        if(ticketCount<=3){
          ...
          const userWallet=await kycuserWalletModel.findOne({ownerId:userId})
          console.log("hello",userWallet);
           ...
           if(userWallet.defaultAmount>=matchData.fee ){
            ...
            await kycuserWalletModel.findByIdAndUpdate(userWallet._id,{$inc:{defaultAmount:-matchData.fee}},{new:true})
            ...
            const x  = tambola.generateTickets(ticketCount) //This generates 100 tambola tickets
            await kycuserWalletModel.findByIdAndUpdate(userWallet._id,{$inc:{ winningAmount:-matchData.fee}},{new:true})
            const matchWallet=await roomWalletModel.findOne({roomId:matchId})
            await roomWalletModel.findByIdAndUpdate(matchWallet._id,{$inc:{walletAmount:matchData.fee}},{new:true})
            await RoomModel.findByIdAndUpdate(matchData._id,{$inc:{ ticketBuyerCount:1}},{new:true})
            res.status(200).json({x})
            ...
            const x  = tambola.generateTickets(ticketCount) //This generates 100 tambola tickets
            await kycuserWalletModel.findByIdAndUpdate(userWallet._id,{$inc:{userAddedAmount:-matchData.fee}},{new:true})
            const matchWallet=await roomWalletModel.findOne({roomId:matchId})
            await roomWalletModel.findByIdAndUpdate(matchWallet._id,{$inc:{walletAmount:matchData.fee}},{new:true})
            await RoomModel.findByIdAndUpdate(matchData._id,{$inc:{ ticketBuyerCount:1}},{new:true})
            res.status(200).json({x})
...
export const claim=async(req,res)=>{
   try {
    ...
    if(matchData.members.indexOf(userId)!==-1 ){
     ...
     await kycuserWalletModel.findOneAndUpdate ({ownerId:userId},{$inc:{winningAmount:firstRowWin}},{new:true})
     ...
     await UserModel.findByIdAndUpdate(userId,{$inc:{totalWinnings:1}},{new:true})   
         ...
         return   res.status(200).json({claimResponse})
           ...
           await kycuserWalletModel.findOneAndUpdate ({ownerId:userId},{$inc:{winningAmount:secondRowWin}},{new:true})
     ...
     await kycuserWalletModel.findOneAndUpdate ({ownerId:userId},{$inc:{winningAmount:thirdRowWin}},{new:true})
    ...
    return   res.status(200).json({claimResponse})
           ...
           await kycuserWalletModel.findOneAndUpdate ({ownerId:userId},{$inc:{winningAmount:cornerWin}},{new:true})
           await UserModel.findByIdAndUpdate(userId,{$inc:{totalWinnings:1}},{new:true})   
           const claimResponse={
            point:cornerWin,
          ...
          return   res.status(200).json({claimResponse})
           ...
           await kycuserWalletModel.findOneAndUpdate ({ownerId:userId},{$inc:{winningAmount:TambolaWin}},{new:true})
           await UserModel.findByIdAndUpdate(userId,{$inc:{totalWinnings:1}},{new:true})   
           const claimResponse={
            point:TambolaWin,
